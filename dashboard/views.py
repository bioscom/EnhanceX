...
from .viewsmtodashboard import *
from .viewsdashboardplotlygraphs import *
...
def opex_report(request, report_year=None, report_week=None):
    try:
        ...
        return handle_weekly_report(request, opex_weekly_Initiative_Report, showOpexThisWeek, showOpexLastWeek, oWeekTW, oWeekLW, oYear, oWeeks, oFunction, year_range)
...
def delivery_report(request, report_year=None, report_week=None):
    try:
        ...
        oWeeks = get_weeks_in_year(oYear)
        oFunction = Workstream.objects.all() 
...
def DeliveryPageLoader(request, queryTW, queryLW, oYear, oFunction, year_range, opexRecogForm, opexRecognition, oWeekTWLW, oWeeks):
    # totalRecordCount=0
    # TotalBankedTW=0
    # FunnelGrowthTW=0
    ...
    #6. Total Banked Value
    TotalBankedTW=float(BankedTW) - float(BankedLW)
...
#except Exception as e:
#    print(traceback.format_exc())
    return render(request, 'management/delivery/delivery.html', {'xfunctions':oFunction, "year_range": year_range, 'oWeeks':oWeeks, 'oWeekTW':oWeekTWLW,
                                                            'record':totalRecordCount, 'FunnelSize':PlannedTW, 'ForecastYTD':ForecastTW,
                                                            'BankedYTD':BankedTW, 'TotalBankedTW':TotalBankedTW, 'FunnelGrowth':FunnelGrowthTW,
                                                            'funnelBelowG3':funnelBelowG3,
                                                            'pipeLineRobustness':pipeLineRobustness, 'oReports':oReports, 'opexRecogForm':opexRecogForm, 
